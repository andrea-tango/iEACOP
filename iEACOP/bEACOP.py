...
class bEACOP(object):  # evolutionary algorithm for complex-process optimization
	...
	def evaluate_local_search(self, z):
	...
	def iterate(self):
		...
		if self.apply_local_search:
			...
			if self.last_best_local==0:
				...
				elif self.local_fit_evals >= self.n2:
						self.apply_local2()
	...
	def solve(self, max_fitness_evaluations=10000, n_individuals=None, fitness_function=None, fitness_args=None, optimization_method="Powell", coeff=0, seed=None, rep=None):
		...
		if rep is None:
			self.fitness_file = self.path_out + os.sep + "bEACOP_fitness"
			self.positions_file = self.path_out + os.sep + "bEACOP_positions"

		else:
			self.fitness_file = self.path_out + os.sep + "bEACOP_fitness_rep%d" % rep
			self.positions_file = self.path_out + os.sep + "bEACOP_positions_rep%d" % rep
