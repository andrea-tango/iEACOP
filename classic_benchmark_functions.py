...
def Quintic( individual):
	...
	accum = 0
	for i in range(D):
		accum += math.fabs(individual[i]**5 + 3*individual[i]**4 + 4*individual[i]**3 + 2*individual[i]**2 - 10*individual[i] -4 )
	return accum
...
def Bohachevsky( individual ):
	...
	accum = 0.0
	for i in range(D-1):
		accum += ( individual[i]**2 + 2*individual[i+1]**2 - 0.3*math.cos(3*math.pi*individual[i]) - 0.4*math.cos(4*math.pi*individual[i+1]) + 0.7 )
	return accum
